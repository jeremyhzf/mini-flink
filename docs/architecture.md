...
│  RuntimeContext · CheckpointCoordinator · SnapshotCallback         │
├───────────────────────────────────────────────────────────────────┤
...
>
> `CheckpointCoordinator` 留在 `runtime` 而非 `checkpoint`，因其持有 `List<SourceTask>`，放入 `checkpoint` 会引入反向依赖。原设计 spec（`docs/superpowers/specs/2026-07-10-mini-flink-design.md`）计划了独立的 `state`/`checkpoint` 包，现已落地。会话窗口、`KeyedProcessOperator` 未实现（推迟）。
...
- `CheckpointCoordinator`（`runtime`）：daemon 线程周期触发 + ack 汇聚 + retained 管理。
- 快照数据类（均 `Serializable`）：`StateSnapshot`/`OperatorState`（`state`）、`SubtaskSnapshot`/`Checkpoint`/`WindowOperatorState`（`checkpoint`）。
